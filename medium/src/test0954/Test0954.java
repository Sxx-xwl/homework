...
class Solution {
    ...
    private static boolean check(int[] arr, boolean[] used, int s, int e) {
        int l = s, r = s + 1;
        while (l < e) {
            ...
            // 需要区分负数区和非负数区的匹配条件不同：
            while (r < e && ((arr[r] < 0 && 2 * arr[r] < arr[l]) || (arr[r] >= 0 && arr[r] < 2 * arr[l]))) {
                r++;
            }
            if (r == e || ((arr[r] < 0 && 2 * arr[r] > arr[l]) || (arr[r] >= 0 && arr[r] > 2 * arr[l]))) {
                return false;
            }
